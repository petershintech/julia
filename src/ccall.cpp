...
#define is_libjulia_func(name) _is_libjulia_func((uintptr_t)&(name), #name)
    ...
    if (is_libjulia_func(jl_array_ptr)) {
        ...
        if (is_libjulia_func(jl_gc_disable_finalizers_internal))
            ctx.builder.CreateStore(ctx.builder.CreateAdd(finh, ConstantInt::get(T_int32, 1)), pfinh);
        else
            ctx.builder.CreateStore(ctx.builder.CreateSub(finh, ConstantInt::get(T_int32, 1)), pfinh);
        return ghostValue(jl_nothing_type);
    }
